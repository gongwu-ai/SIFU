...
对 Trafilatura_01 SIFU runs 的 DNA 文件进行分析：
...
### 问题：DNA 变成了 verbose log

**Run2 (失败, 97行) 的 DNA 特点**：
```
- [DNA-029] 基础设计
- [DNA-030] 工作流
- [DNA-031] R2 improvement
- [DNA-032] R4 refinement
- [DNA-033] R5 pivot
- [DNA-034] R10 exploration
- [DNA-035] R11 pivot
- [DNA-036] R12 refactor
...
### Session: R1-R12 详细记录（12 个 session）
## Misc: 80+ 行的技术细节
```

**Run3 (成功 iter3, 26行) 的 DNA 特点**：
```
- [DNA-001] 使用 Trafilatura 提取内容
- [DNA-002] 输出格式：纯文本
- [DNA-003] 错误处理
...
| 维度 | 预期 | 实际 |
|------|------|------|
| **DNA 内容** | 精简的设计决策 | 详细的迭代日志 |
| **长度** | 10-20 行 | 26-97 行 |
| **更新方式** | 增量修正，保持简洁 | Append everything |
| **用途** | 核心知识库 | 调试日记本 |

### 原因分析

1. **Prompt 没有约束 DNA 格式**
   - 只说"记录设计决策"
   - 没有限制长度或结构
   - Agent 自由发挥，写成了日志
...
3. **长 DNA 可能有害**
   - Agent 在冗长历史中迷失
   - 被自己之前的"失败方向"误导
   - Run2 记录了 12 次失败尝试，反而强化了错误路径

### 改进建议

1. **限制 DNA 长度**
   ```
   规则：DNA 文件不超过 30 行。超过时，精简历史保留核心决策。
   ```

2. **区分决策 vs 日志**
   ```
   ## Decision Rationale (最多 5 条)
   ## Implementation History (最多 3 个 session)
   ```

3. **强制精简**
   ```
   每轮更新时，先读现有 DNA，保留有效信息，删除过时内容。
   ```

### 结论

**当前 SIFU 实现的 compact 机制不完整**：
- DNA 解决了"历史丢失"问题
- 但没解决"历史膨胀"问题
- 需要额外机制控制 DNA 质量和长度

**Paradox**: 更详细的 DNA 反而导致更差的结果。简洁 = 有效。
