...
运行 `claude --print` 实验时，**必须隔离工作目录**，防止 agent 读取当前目录的 CLAUDE.md：

```bash
# 错误: Agent 会读取当前目录的 CLAUDE.md，污染实验
claude --model haiku --print -p "$PROMPT"

# 正确: 用 subshell + cd 隔离到实验工作目录
(cd "$workdir" && claude --model haiku --print -p "$PROMPT")
```

R4 实验因未隔离工作目录，导致所有 agent 读取了 SIFU 项目的 CLAUDE.md，实验数据被污染。R5 修复后发现：SIFU vs Baseline 无差异（40% vs 40%），说明 R4 的 +13% 差异来源于 CLAUDE.md 污染。详见 `docs/2026011403_R5_EXPERIMENT_DESIGN.md`。
